L.Control.Weather = L.Control.extend({
  options: {
    ...
    template: '<div class="weatherIcon"><img src=":iconurl"></div><div>Temperature: :temperature°F</div><div>Humidity: :humidity%</div><div>Wind: :winddirection :windspeed mph</div>',
  ...
  _updateWidget: function(weather) {
    ...
    tpl = tpl.replace(":temperature", Math.round((weather.main.temp*(9/5)) + 32));
    tpl = tpl.replace(":humidity", weather.main.humidity);
    tpl = tpl.replace(":windspeed", Math.round(weather.wind.speed*2.237));
